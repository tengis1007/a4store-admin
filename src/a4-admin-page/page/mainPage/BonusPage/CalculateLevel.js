  ...
  const processedData = useMemo(() => {
    ...
    emanager.forEach((element1) => {
      ...
      zahiral.forEach((element2) => {
        if (
          element1.MemberId + element1.QuantityName === element2.MemberId + element2.QuantityName
        ) {
          count++;
        }
      });
  ...
  const handleDataBase = async () => {
    ...
    if (confirm) {
      try {
        ...
        // Ensure you await the set operation since it's asynchronous
        await set(ref(db, "userInfo/"), sendData);

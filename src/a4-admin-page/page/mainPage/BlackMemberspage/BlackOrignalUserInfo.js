...
const csvConfig = mkConfig({
  filename: `Блэк Үлдсэн худалдан авалт-${dayjs().format("YYYY-MM-DD HH:mm:ss")}`,
  fieldSeparator: ",",
  decimalSeparator: ".",
  columnHeaders: [
    {
...
const Example = () => {
  ...
  const useCreateUser = () => {
    ...
    return useMutation({
      mutationFn: async (user) => {
        const newUserRef = ref(db, "BlackMembers/VipUserInfo");
        await set(newUserRef.push(), user);
        return Promise.resolve();
      },
      onMutate: (newUserInfo) => {
        queryClient.setQueryData(["BlackMembers/VipUserInfo"], (prevUsers) => [
          ...(prevUsers || []),
          {
            ...newUserInfo,
  ...
  function useGetUsers() {
    return useQuery({
      queryKey: ["BlackMembers/VipUserInfo", searchTerm, fetchAll],
      queryFn: async () => {
        try {
          let promoQuery;
          if (fetchAll) {
            // Fetch all data if fetchAll is true
            promoQuery = ref(db, "BlackMembers/VipUserInfo");
          } else if (searchTerm.length === 8) {
            console.log(searchTerm);
            // Only query if searchTerm is defined
            promoQuery = query(
              ref(db, "BlackMembers/VipUserInfo"),
  ...
  const useUpdateUser = () => {
    ...
    return useMutation({
      mutationFn: async (user) => {
        ...
        const userRef = ref(db, `BlackMembers/VipUserInfo/${sendData.id}`);
        await update(userRef, sendData);
        return Promise.resolve();
      },
      onMutate: (newUserInfo) => {
        queryClient.setQueryData(["BlackMembers/userInfo"], (prevUsers) =>
          prevUsers?.map((prevUser) =>
            prevUser.id === newUserInfo.id ? newUserInfo : prevUser
          )
        );
      },
      onSettled: () => queryClient.invalidateQueries(["BlackMembers/VipUserInfo"]), // Refetch users after mutation
    });
  };
  ...
  const useDeleteUser = () => {
    ...
    return useMutation({
      mutationFn: async (userId) => {
        const userRef = ref(db, `BlackMembers/VipUserInfo/${userId}`);
...
export default BlackOrignalUserInfo;

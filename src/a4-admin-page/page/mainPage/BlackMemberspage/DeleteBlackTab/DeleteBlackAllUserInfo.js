...
const csvConfig = mkConfig({
  filename: `Устгасан бүх худалдан авалт-${dayjs().format(
    "YYYY-MM-DD HH:mm:ss"
  )}`,
  fieldSeparator: ",",
  decimalSeparator: ".",
...
const Example = () => {
  ...
  const useCreateUser = () => {
    ...
    return useMutation({
      mutationFn: async (user) => {
        const newUserRef = ref(db, "BlackMembers/delete/userInfo");
        await set(newUserRef.push(), user);
        return Promise.resolve();
      },
      onMutate: (newUserInfo) => {
        queryClient.setQueryData(
          ["BlackMembers/delete/userInfo"],
          (prevUsers) => [
            ...(prevUsers || []),
            {
              ...newUserInfo,
  ...
  function useGetUsers() {
    return useQuery({
      queryKey: ["BlackMembers/delete/userInfo", searchTerm, fetchAll],
      queryFn: async () => {
        try {
          let promoQuery;
          if (fetchAll) {
            // Fetch all data if fetchAll is true
            promoQuery = ref(db, "BlackMembers/delete/userInfo");
          } else if (searchTerm.length === 8) {
            console.log(searchTerm);
            // Only query if searchTerm is defined
            promoQuery = query(
              ref(db, "BlackMembers/delete/userInfo"),
  ...
  const useUpdateUser = () => {
    ...
    return useMutation({
      mutationFn: async (user) => {
        ...
        const userRef = ref(
          db,
          `BlackMembers/delete/userInfo/${sendData.id}`
        );
        await update(userRef, sendData);
        return Promise.resolve();
      },
      onMutate: (newUserInfo) => {
        queryClient.setQueryData(["BlackMembers/delete/userInfo"], (prevUsers) =>
      ...
      onSettled: () =>
        queryClient.invalidateQueries(["BlackMembers/delete/userInfo"]), // Refetch users after mutation
    });
  };
  ...
  const useDeleteUser = () => {
    ...
    return useMutation({
      mutationFn: async (userId) => {
        const userRef = ref(db, `BlackMembers/delete/userInfo/${userId}`);
...
export default DeleteBlackAllUserInfo;
